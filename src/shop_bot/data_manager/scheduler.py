...
async def _maybe_run_daily_backup(bot: Bot):
    ...
    Интервал задаётся парой настроек: backup_interval_days — число, а
    backup_interval_unit — в чём оно считается, в часах или в днях.
    """
    ...
    interval_seconds = max(1, interval_value) * (3600 if unit == "hours" else 24 * 3600)
