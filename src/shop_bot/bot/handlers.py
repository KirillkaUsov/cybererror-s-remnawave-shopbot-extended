...
def get_user_router() -> Router:
    ...
    async def process_trial_key_creation(message: types.Message, host_name: str):
        ...
        try:
            ...
            trial_squad = (get_setting("trial_internal_squad_uuid") or "").strip() or None
    ...
    async def refresh_key_info_internal(bot: Bot, chat_id: int, message_to_edit: types.Message, key_id: int, user_id: int, prompt_message_id: int = None, state: FSMContext = None):
        ...
        try:
            ...
            if remote_user:
                # На 404 панель не отдаёт тело — get_connected_devices_count вернёт None.
                devices = await remnawave_api.get_connected_devices_count(remnawave_api.user_ref(remote_user), host_name=key_data.get('host_name'))
        ...
        try:
            ...
            try:
                ...
                if details and details.get('connection_string'):
                    ...
                    hw_usg = ((await remnawave_api.get_connected_devices_count(remnawave_api.user_ref(details['user']), new_host)) or {}).get('total', 0) if details.get('user') else 0
