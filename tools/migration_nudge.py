...
#
# Ровно 72 часа — это отметка, которую старый бот раньше обслуживал сам.
# Мы её у него забрали (NOTIFY_BEFORE_HOURS там теперь {48, 24, 1}), потому
# что он не видит новую базу: он одинаково торопит и того, кто ещё не
# перешёл, и того, кто уже купил ключ в новом боте. Ближе к концу срока
# напоминания снова его.
WARN_DAYS = 3
...
def text_deadline_b(when: dt.datetime) -> str:
    ...
    Никаких «всё остальное на месте»: подписка как раз не переносится, и
    обещать человеку, что у него уже всё готово, — это ровно то письмо,
    после которого пишут в поддержку с претензией.
    """
    return (
        f"⏳ Ваша подписка заканчивается <b>{when.strftime('%d.%m')}</b>\n\n"
        f"Аккаунт в <b>@{NEW_BOT}</b> у вас уже есть — войдите и выберите тариф.\n\n"
        f"👉 https://t.me/{NEW_BOT}\n\n"
        "Нужно перенести оставшиеся дни — напишите в поддержку "
        f"до {when.strftime('%d.%m')}."
    )
